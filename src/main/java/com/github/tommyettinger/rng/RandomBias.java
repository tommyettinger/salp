...
public class RandomBias implements Serializable {
    ...
    public RandomBias() {
        biases = new LinkedHashMap<String, Double>(32);
        rng = new RNG();
    }
    ...
    public RandomBias(RNG rng, Map<String, Double> mapping, int distribution) {
        ...
        if (mapping == null) {
            ...
            biases = new LinkedHashMap<String, Double>(mapping.size());
            double exp;
    ...
    private double quantile(double expected) {
        switch (distribution) {
            case EXPONENTIAL:
                return exponentialQuantile(expected);
            case TRUNCATED:
                return truncatedQuantile(expected);
            case TRIANGULAR:
                return triangularQuantile(expected);
            case SOFT_TRIANGULAR:
                return softQuantile(expected);
            case BATHTUB_TRUNCATED:
                return bathtubTruncatedQuantile(expected);
            default:
                return mixQuantile(expected);
        }
    }
