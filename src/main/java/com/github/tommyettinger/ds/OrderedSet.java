...
public class OrderedSet<K> implements SortedSet<K>, java.io.Serializable, Cloneable {
    ...
    protected void moveIndexToFirst(final int i) {
        if(size <= 1 || first == i)
            return;
        order.moveToFirst(i);
        if (last == i) {
    ...
    protected void moveIndexToLast(final int i) {
        if(size <= 1 || last == i)
            return;
        order.moveToLast(i);
        if (first == i) {
    ...
    */
    ...
    /**
     * Makes a OrderedSet of T given an array or vararg of T elements. Duplicate items in elements will have
